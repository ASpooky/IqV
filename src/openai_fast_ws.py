...
from pipecat.frames.frames import (
    Frame, StartFrame, AudioRawFrame, InputAudioRawFrame
)
...
def read_config() -> dict:
    try:
        ...
        supported_openai_voices = ["alloy", "ash", "ballad", "cedar", "coral", "echo", "fable", "marin", "nova", "onyx", "sage", "shimmer", "verse"]
        if config.get("voice_id") not in supported_openai_voices:
            logger.warning(f"Voice '{config.get('voice_id')}' is not supported by OpenAI TTS. Falling back to 'alloy'.")
...
async def run():
    while True:
        config = read_config()
        logger.info(f"[pipecat] config: voice={config['voice_id']} name={config['name']}")
        ...
        logger.info(f"[pipecat] listening on ws://{PIPECAT_HOST}:{PIPECAT_PORT} (OpenAI pipeline)")
